...
impl ProxyConnection {
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (request_tx, mut request_rx) = mpsc::channel::<Bytes>(1);
        ...
        let (mut response_tx, response_rx) =
            futures::channel::mpsc::channel::<io::Result<Bytes>>(1);
        let response_rx = StreamReader::new(response_rx);

        // Send an HTTP request, and stream the response body into `response_tx`.
        let pump_incoming = async move {
            let read_response = sender1.send_request(read_request).await?;
        ...
        let pump_outgoing = async move {
            while let Some(chunk) = request_rx.recv().await {
                ...
                let write_response = sender2.send_request(write_request).await?;
        ...
        // Spawn a task to drive the pump futures.
        let pump_task = tokio::spawn(join(pump_incoming, pump_outgoing)).abort_handle();
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        // exchange HTTP headers and get status code & start streaming data in the background
        let read_request = create_read_request(config, session_id, Either::Left(Empty::new()));
        let read_request = tokio::spawn(sender.send_request(read_request));
        ...
        let pump_task = tokio::spawn(async move {
            while let Some(chunk) = request_rx.recv().await {
                let request_body = Either::Right(Full::new(chunk));
                let write_request = create_write_request(&config, session_id, request_body);
                let write_response = sender.send_request(write_request).await?;
        ...
        let read_response = read_request.await.unwrap()?;
        if !read_response.status().is_success() {
            return Err(Error::HttpStatusCode(read_response.status()));
        }

        // convert the response `Incoming` to an `AsyncRead`
        let response_rx = read_response
            .into_body()
            .into_data_stream()
